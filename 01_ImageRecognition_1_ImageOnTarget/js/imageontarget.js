var World = {
	...
	createOverlays: function createOverlaysFn() {
		/*
			First an AR.ImageTracker needs to be created in order to start the recognition engine. It is initialized with a
			AR.TargetCollectionResource specific to the target collection that should be used. Optional parameters are passed
			as object in the last argument. In this case a callback function for the onTargetsLoaded trigger is set. Once the
			tracker loaded all its target images, the function worldLoaded() is called.

			Important: If you replace the tracker file with your own, make sure to change the target name accordingly.
			Use a specific target name to respond only to a certain target or use a wildcard to respond to any or a certain group 
			of targets.
		*/
			/* Create overlay for page one */
		var imgOne = new AR.ImageResource("01_ImageRecognition_1_ImageOnTarget/assets/imageOne.png");
		...
		AR.logger.warning("past target collection resource");
	try{
        this.tracker = new AR.ImageTracker(this.targetCollectionResource, {
            onTargetsLoaded: this.worldLoaded,
			// onError: function(err){
			// 	AR.logger.info(`Error in image tracker: ${err}`);
			// }
        });
	} catch(er){
			AR.logger.error('image tracker error: ' + er.message);		
	}
		/*
	...
	worldLoaded: function worldLoadedFn() {
		...
		document.getElementById('loadingMessage').innerHTML =
			...
			"<div" + cssDivRight + "><img src='01_ImageRecognition_1_ImageOnTarget/assets/surfer.png'></img></div>";
...
AR.logger.activateDebugMode();
	// AR.logger.warning(`Logging has started`);
World.init();
